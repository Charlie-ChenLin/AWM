...
- **Robustness**: Proven effective against Supervised Fine-Tuning (SFT), extensive Continued Pre-Training (CPT), Reinforcement Learning (RL), multi-modal extensions, pruning, and upcycling.
- **High Fidelity**: Achieves perfect AUC and pAUC scores on our comprehensive testbed, ensuring extremely low false-positive rates to prevent false accusations of model theft.
- **Training-Free**: AWM analyzes model weights directly without any additional training, preserving the model's original performance.
- **Efficient**: The entire similarity computation completes within 30 seconds on a single NVIDIA 3090 GPU.
...
├── models.txt              # List of Hugging Face model IDs for download
...
1.  **Clone the Repository**
    ```bash
    git clone https://github.com/your-repo/awm-fingerprint.git
    cd awm-fingerprint/analysis_code
    ```
...
1.  **Download Models**: Use `huggingface-cli` to download the two models you want to compare. For example, Llama-2-7B and Vicuna-7B.
    ```bash
    huggingface-cli download meta-llama/Llama-2-7b-hf --local-dir ../checkpoints/Llama-2-7b-hf
    huggingface-cli download lmsys/vicuna-7b-v1.5 --local-dir ../checkpoints/vicuna-7b-v1.5
    ```

2.  **Run the Analysis**: Use the `run_analysis.py` script with the `--model_paths` argument pointing to the two model directories.
    ```bash
    python run_analysis.py --model_paths ../checkpoints/Llama-2-7b-hf ../checkpoints/vicuna-7b-v1.5
    ```
